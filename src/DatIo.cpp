...
//		   OUTPUT: no ouptus
//
//    DESCRIPTION: the entirety of the library relies on tabulated values of beta_min and beta_max for each even
//				   value of number of (quadrature) nodes and for this reason it is shipped with those values in  
//				   the 'data/TabulatedErrorValues.csv' file. However should the file be corrupted or deleted this
//				   routine, when triggere by the absence of the file itself, reconstructs such table from scratch.
...
//――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――
//
//       FUNCTION: [n, {lambda_min, lambda_max}] = getInputData(muntz_sequence, coeff_sequence)
//                
...
//				   - lambda_min = minimum exponent in the input "muntz_sequence" (strictly greater than -1)
//				   - lambda_max = maximum exponent in the input "muntz_sequence"
//
//    DESCRIPTION: the user-input polynomial is provided to the library via a main.cpp function in a amendable file;
//                 the polynomial itself is specified via a unordered sequence of coefficients and exponents of the
//                 various monomials in the polynomial. Once those input are read, checks have to be made in order
//                 to validate the proper functioning of the library; those are:
//				   - the number of exponents and the number of coefficients coincide (otherwise the program exits);
//				   - the input polynomial is at least a binomial (otherwise the routine further CLI user-input is 
//					 required, see lines 86~93 in the 'src/MonMap.cpp' file); 
//				   - lambda_min > -1 (otherwise the program exits);
//                 Once those checks are ran the exponents' sequence is sorted locally and lambda_min/lambda_max
//                 are thus identified and outputted alongside the associated number of nodes computed by the 
//                 function 'computeNumNodes' (see lines 110~175 in the 'src/MonMap.cpp' file).
...
//
//		   OUTPUT: - n_min = minimum possible (even) number of nodes from the 'data/TabulatedErrorValues.csv' file
//				   - beta_min = minimum value for the exponent of the user-input polynomial after the map
//				   - beta_max = maximum value for the exponent of the user-input polynomial after the map
//
//    DESCRIPTION: the monomial transformation gamma: [0,1] -> [0,1] is uniquely identified by its order r which
//                 in turn requires the knowledge of beta_min/beta_max, alongside lambda_min/lambda_max, to be
//                 computed (see lines 178~211 in the 'src/MonMap.cpp' file). This method scans the tabulated vales
//                 in the 'data/TabulatedErrorValues.csv' file to extract the beta_min/beta_max and n_min required
//                 by the monomial quadrature rule according to the specified number of nodes as either computed by
//                 the function 'computeNumNodes' (see line 243) or provided as user-input (see lines 168~199, 247).
...
//――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――
//
//       FUNCTION: {quadratures, errors} = degradeData([J, {new_x, new_w, old_x, old_w}], muntz_sequence, coeff_sequence)
//                
	...
	if(float50_error <= epsilon)
	{
    	...
    	if(float128_error <= epsilon && float128_error != 0)
    	{
    		...
    		if(double_error <= epsilon && double_error != 0)
    		{
    			...
    			exportData(quad_params, output_data, collected_data);
    		}
    		else
    		{
...
//――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――
//
//       FUNCTION: exportData({lambda_min, lambda_max}, [n_min, {beta_min, beta_max}], [], {post_map_integral, pre_map_integral}, r)
//                
...
//                 - {post_map_integral, pre_map_integral} = output of function 'computeQuadGl'
//                 - r = output of function 'computeOrder'
//
//		   OUTPUT: no outputs
//
//    DESCRIPTION: once the monomial quadrature rule is completed the resulting data is streamed in output text
//                 files inside the output subdirectory created, by this routine, within the calling directory
//                 of the executable of this library. The output data is splitted in three files with 'Results.txt'
//                 containing recap informations of the execution (including the resulting integral) and 'Nodes.txt'
//                 and 'Weights.txt' containg the classic and new G-L nodes and weights respectively.
...
void exportData(const std::tuple<double, std::vector<float50>, std::vector<float50>, std::vector<float50>, std::vector<float50>>& quad_params, const std::vector<type>& output_data, const std::vector<double>& collected_data)
{
	...
	std::cout << " ――――――――――――――――――――――――――――――――――――――――――――――――――"
			  << "\n ** Using " << typeid(type).name() << " format for nodes and weights"
...
template void exportData<float50>(const std::tuple<double, std::vector<float50>, std::vector<float50>, std::vector<float50>, std::vector<float50>>& quad_params, const std::vector<float50>& output_data, const std::vector<double>& collected_data); // Template mock instantiation for non-inline function
template void exportData<float128>(const std::tuple<double, std::vector<float50>, std::vector<float50>, std::vector<float50>, std::vector<float50>>& quad_params, const std::vector<float128>& output_data, const std::vector<double>& collected_data); // Template mock instantiation for non-inline function
template void exportData<double>(const std::tuple<double, std::vector<float50>, std::vector<float50>, std::vector<float50>, std::vector<float50>>& quad_params, const std::vector<double>& output_data, const std::vector<double>& collected_data); // Template mock instantiation for non-inline function
//――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――
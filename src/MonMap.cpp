...
//			           - n = see (18) in [1]
//								 - envelope = string flag that specifies whether the estimate must be enveloped (to avoid finite arithmetic infinities) or not
//
//		     OUTPUT: - E_n = exact asymptotic estimate of the G-L quadrature error computed using (13) and (18) in [1]
//
//    DESCRIPTION: in order for the library to generate tabulated values for beta_min/beta_max as functions of n
//								 the exact estimates of the G-L quadrature error must be computed with high-precision. Such an 
//								 a-priori estimate is generally known as results of complex analysis (see [2]) however in [1] a more
//                 accurate form has been devised in formulae (13) and (18) which are implemented in this routine to
//                 compute the aformentioned error estimate.
//
//		  REFERENCE: [1] = Lombardi Guido - Design of quadrature rules for Müntz and Müntz-logarithmic polynomials using monomial transformation,
//																			  Int. J. Numer. Meth. Engng., 80: 1687-1717,
//                                        https://doi.org/10.1002/nme.2684.
//                 [2] = Donaldson J.D., Elliott D. - A unified approach to quadrature rules with asymptotic estimates of their remainders,
//                                                    SIAM Journal on Numerical Analysis 1972; 9(4):573–602,
...
//                
//          INPUT: - lambda_min = minimum exponent in the "muntz_sequence" input of function 'getInputData'
...
//
//    DESCRIPTION: the monomial quadrature rule is a pre-processing of the G-L nodes & weights for those polynomials
//								 characterised by an arbitrarly large gap between the terms of minimum and maximum degree. There
//                 might be cases however in which the user wants to integrate singular monomials; in those cases
//								 the library will require an additional, non-constant, term to be added to the monomial. It does so
//							   by either allowing the user to either manually input the exponent of the additional term from the CLI 
//								 (see lines 180~184 in the src/DatIo.cpp file) or to specify the maximum number of quadrature nodes
//								 to use in its application. In this last instance the the following function automatically generates
//                 the resulting exponent of the additional term (lambda_max).
...
//                
//          INPUT: - lambda_min = minimum exponent in the input "muntz_sequence" (strictly greater than -1)
//			           - lambda_max = maximum exponent in the input "muntz_sequence"
//
//		     OUTPUT: - n = number of (quadrature) nodes computed as solution of equation (62) in [1]
//
//    DESCRIPTION: once the exponents in the terms with minimum and maximum degree in the user-input polynomial
//                 have been determined, this method implements formula (62) in [1], which is a 7-th degree
//                 polynomial in n, and extract its only real root whose integer floor will then be the minimum
//                 possible number of (quadrature) nodes to be used in G-L formula to achieve double precision
//                 (the polynomial solver itself is a class method implemented in GSL-GNU Scientific Library).
//
//		  REFERENCE: [1] = Lombardi, Guido - Design of quadrature rules for Müntz and Müntz-logarithmic polynomials
//																			   using monomial transformation,  Int. J. Numer. Meth. Engng., 80: 1687-1717
//                                         https://doi.org/10.1002/nme.2684
...
//
//    DESCRIPTION: the order of the monomial transformation for the new nodes and weights of the G-L quadrature
//                 formula is computed as a linear interpolation beetween r_min and r_max reported in (63) of [1].
//
//		  REFERENCE: [1] = Lombardi, Guido - Design of quadrature rules for Müntz and Müntz-logarithmic polynomials
//																			   using monomial transformation,  Int. J. Numer. Meth. Engng., 80: 1687-1717
//                                         https://doi.org/10.1002/nme.2684
...
//
//		     OUTPUT: - J = jacobian of the affine map phi: [a,b] -> [-1,1] of the Gauss-Legendre quadrature formula
//								 - {new_x, new_w} = new set of G-L quadrature nodes (x) and weights (w) following the monomial map
//								 - {old_x, old_w} = classical set of G-L quadrature nodes (x) and weights (w) in [-1, 1]
//
//    DESCRIPTION: once the transformation order is available, the monomial map itself is constructed according to 
//                 (55) in [1]; of course, prior to the monomial map is applied, the G-L nodes and weights have to
//								 be mapped from the user-input interval I = [a,b] to [-1, 1]. This is implemented in this routine
//								 which also carries the jacobian of the affine map among the outputs since it is the multiplication
//                 coefficient of the integral itself (thus needed by the function 'computeQuadGl'). Furthermore
//                 the classic G-L nodes and weights are outputted as well so that comparisons ca be made between
//                 the traditional G-L quadrature integral and the monomial rule quadrature integral.
...
//			           - muntz_sequence = sequence of real exponents of the user-input polynomial
...
//
//    DESCRIPTION: every interpolatory quadrature rule approximates the definite integral by means of a weighted
//                 sum of the kernel's values on specific points along the integration interval (i.e. nodes);
//                 an interpolatory Gaussian quadrature formula is a quadrature rule whose nodes corresponds to the
//                 roots of a polynomial that is orthogonal in the integration interval to the weight function
//                 of the kernel. A Gauss-Legendre quadrature formula with n+1 nodes is a Gaussian formula for 
//                 which the nodes corresponds to the roots of the Legendre n-th degree polynomial that is
//                 orthogonal to the weight function w(x) = 1 in [-1, 1]. This routine implements the G-L
//                 quadrature formula provided classical and new G-L nodes and weights.
...
//――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――
//
//       FUNCTION: error = computeError({post_map_quadrature, pre_map_quadrature}, muntz_sequence, coeff_sequence, I)
//                
//          INPUT: - {post_map_quadrature, pre_map_quadrature} = output of function 'computeQuadGl'
//			           - muntz_sequence = sequence of real exponents of the user-input polynomial
//			           - coeff_sequence = sequence of real coefficients of the user-input polynomial
//                 - I = [a,b] = interval of integration of the user-input polynomial (a < b)
//
//		     OUTPUT: - error = relative error of the G-L quadrature computed with the new nodes and weights
//
//    DESCRIPTION: let I_n be the numerical integral calculated using the G-L quadrature rule (outputed by function
//                 'computeQuadGl') and I_ex be the exact (analytic) value of such integral then the relative error
//                 of the quadrature can be computed a-posteriori as R_n = |I_ex - I_n|/|I_ex|. Such computation
//                 is implemented in this routine with the exact integral being as precise as the user-input
//                 polynomial is.

...
// (SEE LINES 18~39 IN 'src/MonMap.cpp') Computes the optimal lambda_max when the input polynomial is a monomial and a maximum number of nodes is required
float128 computeLambdaMax(float128& lambda_min, int num_nodes);

// (SEE LINES 55~75 IN 'src/MonMap.cpp') Computes the number of minimum quadrature nodes by finding the real root of the 7-th degree polynomial equation in (62)
int computeNumNodes(const float128& lambda_min, const float128& lambda_max);

// (SEE LINES 117~132 IN 'src/MonMap.cpp') Computes the order (r) of the monomial map as a linear interpolation of r_min and r_max
double computeMapOrder(const std::vector<float128>& lambdas, const std::vector<float128>& betas);

// (SEE LINES 176~195 IN 'src/MonMap.cpp') Computes the new nodes and weights of the G-L formula
...
// (SEE LINES 298~324 IN 'src/MonMap.cpp') Computes the numerical integral with G-L quadrature formula
...
// (SEE LINES 352~374 IN 'src/MonMap.cpp') Computes the a-posteriori relative error of the G-L quadrature using the new nodes and weights
